...
<div class="container">
<h2 style="color:rgb(75, 163, 235);text-align:center;">THÔNG TIN TOUR</h2>
  ...
  <table class="table">
    ...
    <tbody >
      <?php
        ...
        $stt = 1;
        echo "<form method='post' action='capnhat.php' >";
            while($row = mysqli_fetch_array($result)){
            
            
                echo "<tr style='text-align: middle;'>";
                    echo "<td>$stt</td>";
                    echo "<td style='width: 110px;'>".$row['Matour']."</td>";
                    echo "<td>".$row['Tentour']."</td>";
                    echo "<td>".$row['Mamien']."</td>";
                    echo "<td>".$row['Manv']."</td>";
                    echo "<td>".number_format($row['Giatour'])." VND</td>";
                    echo "<td><img class ='img-fluid' style ='height: 70px; width: 80px;' src='".$row['Hinh']."'/></td>";
                  ...
                  //  echo "<td><button type ='submit' name='xoa'class='btn btn-primary timkiem' ><i class='fa fa-trash' aria-hidden='true'></i></button></td>";
                   echo "<td><a href='capnhat.php?item=$row[Matour]'  class='btn btn-primary timkiem' ><i class='fa fa-pencil-square' aria-hidden='true'></i></a></td>";
                   echo "<td><a href='xoa.php?item=$row[Matour]' onClick='return ConfirmDelete()'  class='btn btn-primary timkiem' ><i class='fa fa-trash' aria-hidden='true'></i></a></td>";
                  // echo '<td><input type ="submit" style="text-onClick='return ConfirmDelete()'align: center;" name= "xoa" value = "<i class="fa fa-trash" aria-hidden="true"></i>" class="btn btn-primary timkiem"/></td>';
                echo "</tr>";
                $stt++;
            }
